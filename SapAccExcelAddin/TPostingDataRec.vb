Public Class TPostingDataRec
    ...
    Public Sub New(ByRef pDic As Dictionary(Of String, SAPCommon.TField), ByRef pPar As SAPCommon.TStr, Optional pEmpty As Boolean = False, Optional pEmptyChar As String = "#")
    ...
    Private Function initArrays(ByRef pPar As SAPCommon.TStr)
        Dim aNonKeyFields As String = If(pPar.value("GEN", "NON_KEY_STR") <> "", CStr(pPar.value("GEN", "NON_KEY_STR")), "")
        If Not String.IsNullOrEmpty(aNonKeyFields) Then
            aNonKeyArray = aNonKeyFields.Split(",")
        End If
        Dim aValueFields As String = If(pPar.value("GEN", "VALUE_STR") <> "", CStr(pPar.value("GEN", "VALUE_STR")), "Amount")
        If Not String.IsNullOrEmpty(aNonKeyFields) Then
